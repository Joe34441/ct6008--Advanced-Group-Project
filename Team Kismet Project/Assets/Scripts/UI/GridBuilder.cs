...
public class GridBuilder : GridCell
{
	...
	public override void OnRecycled()
	{
		...
		if (transform.parent)
		{
			...
			if (!_resizeWidth) startW = (rt.anchorMax.x - rt.anchorMin.x) * ((RectTransform) transform.parent).rect.width;
			if (!_resizeHeight) startH = (rt.anchorMax.y - rt.anchorMin.y) * ((RectTransform) transform.parent).rect.height;
		}

		minLayoutWidth = startW;
		minLayoutHeight = startH;

		//do not call UpdateRect() in Awake(), call first update
	}
	...
	public void BeginUpdate()
	{
		if (_currentChildren.Count != transform.childCount)
		{
			...
			for(int c = 0; c < transform.childCount;)
			{
				...
				if (po != null) ObjectPool.Recycle(po);
				else c++; //non pooled object, skip it
			}
		}
	...
	{
		if (spc > 0) AddRow(GetSpacerPrefab(), (GridCell spacer) => { spacer.width = 0; spacer.height = spc; });
	}

	public void AddCellSpacing(float spc)
	{
		if (spc > 0) AddCell(GetSpacerPrefab(), (GridCell spacer) => { spacer.width = spc; spacer.height = 0; });
	}
	...
	private void LayoutCell(GridCell cell)
	{
		...
		_nextX += cell.width;
		if (_nextX > _rowWidth) _rowWidth = _nextX;

		if (cell.height > _rowHeight) _rowHeight = cell.height;

		if (cell.endRow) MoveNextRow();
	}
	...
	private T Add<T>(T prefab, float x, float y) where T : GridCell
	{
		...
		else
		{
			...
			else _currentChildren.Add(panel);
		}
	...
	void Update()
	{
		if (_autoResize)
		{
			if (_resizeWidth || _resizeHeight)
			{
				...
				if (_currentChildren.Count > 0) UpdateRect(_rowWidth, -_nextY); // Make sure content pane is resized if needed!
			}
		}
	}
}
